...
``DESTRUCTIVE`` so Rift can gate it (allow / ask / deny).
"""
...
# tools that mutate the workspace or the machine; gated by Rift
DESTRUCTIVE = {
    "write_file",
    "apply_patch",
    "run_command",
...
def describe_environment(workspace: Path) -> str:
    ...
    lines = [
        ...
        f"- Python: {sys.executable} ({platform.python_version()}) is the Rift runtime; "
...
def find_project_python(workspace: Path) -> str:
    """The project's own interpreter, not Rift's.

    Rift runs in its own environment (``sys.executable``), which may
    ...
    dependencies are visible. Falls back to the Rift interpreter when no
    venv is present.
    """
...
class WorkspaceTool(BaseTool):
    ...
    def _snapshot_path(self) -> Path:
        ...
        return self.workspace_root / ".rift" / "state"
    ...
    def _index_snapshot(self, snap: Path) -> None:
        ...
        for f in self.walk(self.workspace_root):
            if not f.is_file():
                continue
            rel = self.rel(f)
            if rel.startswith((".rift/", ".harness/")):
    ...
    def _paths_changed_since(self, slot: str) -> list[str]:
        ...
        for f in self.walk(self.workspace_root):
            rel = self.rel(f)
            if rel.startswith((".rift/", ".harness/")) or rel in saved:
                continue
    ...
    def _apply_snapshot(self, slot: str) -> str:
        ...
        for f in self.walk(self.workspace_root):
            rel = self.rel(f)
            if rel not in saved and not rel.startswith((".rift/", ".harness/")):
                try:
                    f.unlink()
                except OSError:
                    pass
