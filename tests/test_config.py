...
def test_find_config_uses_rift_config_dir(tmp_path, monkeypatch):
    monkeypatch.delenv("RIFT_CONFIG", raising=False)
    monkeypatch.delenv("HARNESS_CONFIG", raising=False)
    if os.name == "nt":
        monkeypatch.setenv("APPDATA", str(tmp_path))
        expected = tmp_path / "rift" / "providers.yaml"
    else:
        monkeypatch.setenv("HOME", str(tmp_path))
        expected = tmp_path / ".config" / "rift" / "providers.yaml"
    assert providers.find_config() is None  # nothing there yet
    expected.parent.mkdir(parents=True)
    expected.write_text("providers: {}\n", encoding="utf-8")
    assert providers.find_config() == expected  # found in the Rift directory
