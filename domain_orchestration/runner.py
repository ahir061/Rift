...
class RunCollector:
    ...
    def __init__(self, run_id: str, agent: Rift) -> None:
        self.run_id = run_id
        self.agent = agent
...
def run_to_result(run_id: str, agent: Rift, query: str) -> RunResult:
    return RunCollector(run_id, agent).run(query)


def iter_run(agent: Rift, query: str) -> Iterator[dict[str, Any]]:
