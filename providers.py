"""Provider layer: which model service Rift talks to.
...
3. Environment variables    (.env / RIFT_*, legacy HARNESS_* / OLLAMA_*, ...)
...
Rift never looks at the config file and uses Ollama's .env settings.

Rift speaks three wire protocols: `ollama`, `openai` (and any
...
Config file locations, first match wins: --config, $RIFT_CONFIG,
%APPDATA%/rift/providers.yaml (or ~/.config/rift/providers.yaml).
The current directory is not searched: the config belongs to Rift, so
...
A provider entry's default model is its `default:` field; Rift uses it
...
list and change the default from the command line:
  rift --provider openrouter --add <model-id>
  rift --provider openrouter --set-default <model-id>
"""
...
# Provider-specific key env vars, consulted before the generic Rift key.
_API_KEY_ENVS = {
...
def api_key_from_env(provider: str) -> str | None:
    """Best API key: provider-specific, then Rift, then the legacy name."""
    for var in _API_KEY_ENVS.get(provider, ()) + ("RIFT_API_KEY", "HARNESS_API_KEY"):
...
def default_config_path() -> Path:
    """Where Rift's providers.yaml lives (created there if missing)."""
    if os.name == "nt" and os.environ.get("APPDATA"):
        return Path(os.environ["APPDATA"]) / "rift" / "providers.yaml"
    return Path.home() / ".config" / "rift" / "providers.yaml"


def find_config(explicit: str | None = None) -> Path | None:
    """Locate Rift's providers.yaml: --config, then $RIFT_CONFIG,
    then the user-level Rift config directory, with the old Harness location
    as a compatibility fallback. The current directory is never searched —
    the config belongs to Rift, not to wherever you
    happen to run it."""
    candidates: list[Path] = []
    if explicit:
        candidates.append(Path(explicit).expanduser())
    env_path = os.environ.get("RIFT_CONFIG") or os.environ.get("HARNESS_CONFIG")
    if env_path:
    ...
    *,
    ...
    resolved_model = (
        _first(
            ...
            os.environ.get("RIFT_MODEL") or os.environ.get("HARNESS_MODEL"),
            legacy_model_env,
    ...
    resolved_base = _first(
        ...
        os.environ.get("RIFT_BASE_URL") or os.environ.get("HARNESS_BASE_URL"),
        legacy_base_env,
