#!/usr/bin/env python3
"""Rift command line: an interactive coding agent, or an API server.

    rift                        # REPL in the current directory (ollama)
    rift "add type hints"       # one task, then exit
    rift chat --provider openai # use OpenAI (or any compatible endpoint)
    rift serve --provider openrouter --config providers.yaml
    rift serve --port 8000      # OpenAI-compatible API
...
from rift import MODES, RETRY_LIMIT, Rift, console, normalize_base_url
...
def add_provider_to_config(path: Path, name: str, opts: dict) -> None:
    ...
    with path.open("w", encoding="utf-8") as fh:
        fh.write("# Named providers for Rift. Read only when --provider NAME is given.\n")
        fh.write("# Without --provider Rift uses ollama from .env.\n")
        fh.write(block + "\n")
...
def run_init_provider(args: argparse.Namespace) -> None:
    ...
    ``rift init-provider`` asks for each value; ``rift init-provider
    ...
    console.print(
        f"use it with: [bold]rift --provider {name}[/bold] "
...
def run_add_model(args: argparse.Namespace) -> None:
    """`rift --provider NAME --add MODEL` — add a model to the provider's
    ...
    if args.provider not in PROVIDERS and not entry.get("base_url"):
        console.print(
            "[dim]this provider has no base_url — set one in the YAML or re-run with rift init-provider[/dim]"
        )
    console.print(f"use it with: [bold]rift --provider {args.provider} --model {args.add}[/bold]")


def run_set_default_model(args: argparse.Namespace) -> None:
    """`rift --provider NAME --set-default MODEL` — set the provider's
    ...
    available set, `default:` is which one Rift uses). The provider
    ...
    console.print(
        f"use it with: [bold]rift --provider {args.provider}[/bold] (defaults to {new}; --model overrides per run)"
    )
...
def provider_config(args: argparse.Namespace) -> tuple[Provider, Path | None]:
    ...
    if args.provider and cfg_path is None:
        # --provider given but no providers.yaml found anywhere: Rift
        ...
        console.print(
            ...
            "  checked: $RIFT_CONFIG, %APPDATA%\\rift\\providers.yaml,\n"
            "           ~/.config/rift/providers.yaml\n"
            "  fix:     put your providers.yaml in one of those places, or run\n"
            f"           [bold]rift --provider {args.provider} "
            "--config PATH[/bold]"
        )
...
def common_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        default=None,
        help="providers.yaml (default: $RIFT_CONFIG, "
        "then %%APPDATA%%\\rift\\providers.yaml or "
        "~/.config/rift/providers.yaml)",
    )
    parser.add_argument(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=Path(sys.argv[0]).stem or "rift",
        description="Rift — a lightweight agentic coding harness.",
    )
    parser.add_argument("--version", action="version", version=f"Rift {VERSION}")
    ...
    initp.add_argument(
        "--config", metavar="PATH", default=None, help="target file (default: the Rift config location)"
    )
...
def print_stats(agent: Rift) -> None:
...
def make_agent(args: argparse.Namespace, workspace: Path, prov: Provider, stream: bool = True) -> Rift:
    try:
        return Rift(
            model=prov.model,
            workspace=workspace,
...
def startup_panel(
    ...
    console.print(
        Panel.fit(
            ...
            "[bold bright_cyan]██║  ██║██║██║        ██║   [/bold bright_cyan]\n"
            "[bold bright_cyan]╚═╝  ╚═╝╚═╝╚═╝        ╚═╝   [/bold bright_cyan]\n"
            "[dim]Built by Ahir · github.com/ahir061[/dim]\n\n"
...
def run_serve(args: argparse.Namespace) -> None:
    ...
    base_url = normalize_base_url(prov.endpoint)
    prov.base_url = base_url
    auth_key = args.api_key or pick_env("RIFT_API_KEY", "HARNESS_API_KEY")  # bearer auth for /v1
